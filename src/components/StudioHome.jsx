    ...
    <div className="space-y-16 pb-12 pl-4 pr-4 animate-fade-in text-slate-100">
      ...
      <div className="relative overflow-hidden rounded-3xl border border-slate-800 shadow-2xl min-h-125 flex items-center">
        ...
        <div className="relative z-10 max-w-2xl p-8 sm:p-12 lg:p-16 space-y-6">
          <div className="inline-flex items-center space-x-2 px-3 py-1 rounded-full text-xs font-semibold bg-cyan-500/10 text-cyan-400 border border-cyan-500/20 backdrop-blur-md">
            ...
            <span>{t('booking_slots_alert', 'Now Booking Online Slots For This Week')}</span>
          </div>
          ...
          <p className="text-slate-300 text-sm sm:text-base leading-relaxed drop-shadow-md">
            {t('hero_desc', 'Experience ultra-premium automotive detailing. From multi-stage machine paint corrections to certified 9H nanotech ceramic coatings, we treat your vehicle to elite aesthetic and protective standards.')}
          </p>
          ...
          <div className="pt-4 flex flex-wrap gap-4">
            ...
            >
              {vehicleCount === null 
                ...
                : t('btn_garage_count', 'Access My Garage ({{count}})', { count: vehicleCount })}
            </button>
          </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {[
            { 
              ...
              value: vehicleCount === null ? t('val_dash_garage_signin', 'Sign in to link vehicles') : vehicleCount > 0 ? t('val_dash_garage_count', '{{count}} Saved Vehicles', { count: vehicleCount }) : t('val_dash_garage_empty', '0 Vehicles Linked'), 
              description: t('desc_dash_garage', 'Add cars to build custom treatments'), 
              icon: '🚗', 
            ...
            { 
              ...
              value: bookingCount === null ? t('val_dash_bookings_signin', 'Sign in to view reservations') : bookingCount > 0 ? t('val_dash_bookings_count', '{{count}} Pending Orders', { count: bookingCount }) : t('val_dash_bookings_empty', 'No Active Bookings'), 
              description: t('desc_dash_bookings', 'Your scheduled studio sessions'), 
              icon: '📅', 
            ...
            { 
              ...
              value: t('val_dash_packages_count', '{{count}} Studio Menus', { count: serviceCount }), 
              description: t('desc_dash_packages', 'Premium treatment options open'), 
              icon: '✨', 
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center bg-slate-900/30 p-6 sm:p-8 rounded-3xl border border-slate-900/80">
        <div className="lg:col-span-5 space-y-4">
          ...
          <p className="text-xs text-slate-400 leading-relaxed">
            {t('slider_desc', 'Drag the interactive center slider left and right to observe how our signature dual-action micro-compounding stage slices away swirl marks, spiderweb surface abrasions, and dull oxidative weathering to reveal deep optical clarity.')}
          </p>
          <div className="pt-2 flex flex-col sm:flex-row gap-y-2 sm:gap-x-6 text-xs font-mono text-slate-400">
            <span className="flex items-center gap-1.5">
              <span className="w-2 h-2 rounded-full bg-red-500"></span> {t('metric_swirls', 'Defect Swirl Removal: 95%+')}
            </span>
            <span className="flex items-center gap-1.5">
              <span className="w-2 h-2 rounded-full bg-cyan-400"></span> {t('metric_gloss', 'Mirror Gloss Index: +42%')}
            </span>
          </div>
        </div>
        ...
        <div className="lg:col-span-7 bg-slate-950 border border-slate-800 p-3 rounded-2xl shadow-2xl relative select-none w-full">
          ...
          <div className="mt-4 flex items-center space-x-4 px-1">
            <span className="text-[10px] font-mono text-slate-500 font-bold">{t('slider_min_label', 'BEFORE')}</span>
            <input 
              type="range" 
              min="0" 
              max="100" 
            ...
            />
            <span className="text-[10px] font-mono text-cyan-400 font-bold">{t('slider_max_label', 'AFTER')}</span>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-6">
        <div className="text-center max-w-xl mx-auto space-y-1">
          <h2 className="text-2xl font-black text-white tracking-tight">{t('standards_heading', 'The Professional Studio Standard')}</h2>
          <p className="text-xs text-slate-400">{t('standards_subheading', 'Why premium automotive care requires scientific precision over automated car wash bays.')}</p>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {[
            { key: 'wash', img: 'https://images.unsplash.com/photo-1607860108855-64acf2078ed9?auto=format&fit=crop&w=400&q=80' },
            { key: 'prep', img: 'https://images.unsplash.com/photo-1563720223185-11003d516935?auto=format&fit=crop&w=400&q=80' },
            { key: 'experts', img: 'https://images.unsplash.com/photo-1507136566006-cfc505b114fc?auto=format&fit=crop&w=400&q=80' },
            { key: 'bays', img: 'https://images.unsplash.com/photo-1619642751034-765dfdf7c58e?auto=format&fit=crop&w=400&q=80' }
          ].map((item, idx) => (
      ...
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-900 pb-4">
          <div>
            <h2 className="text-xl font-black text-white tracking-wide">{t('reviews_heading', 'Client Experiences')}</h2>
            <p className="text-xs text-slate-400 mt-0.5">{t('reviews_subheading', 'Real feedback from verified luxury vehicle and exotic fleet owners.')}</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {[
            { author: t('rev_author_1', 'Marcus V. (Porsche GT3 RS)'), text: t('rev_text_1', 'The paint correction restored a level of clarity I did not think was possible. Reflections look like liquid mirror. Highly recommend their ceramic packages.'), rating: '⭐⭐⭐⭐•' },
            { author: t('rev_author_2', 'Elena R. (Tesla Model S Plaid)'), text: t('rev_text_2', 'Absolute perfection. The interior steam extraction completely pulled out all tracking dust. Booking appointments directly through the profile panel was seamless.'), rating: '⭐⭐⭐⭐•' },
            { author: t('rev_author_3', 'Dave K. (BMW M3 Coupe)'), text: t('rev_text_3', 'Flawless treatment. Zero micro-scratch webbing left behind. The climate-controlled clean-room curation bay shows their unmatched attention to details.'), rating: '⭐⭐⭐⭐•' }
          ].map((rev, idx) => (
            <div key={idx} className="bg-slate-900/30 border border-slate-800/60 rounded-2xl p-6 flex flex-col justify-between space-y-4 shadow-sm">
              <div className="space-y-2">
                <div className="text-xs text-amber-400 font-mono tracking-tight">⭐⭐⭐⭐⭐</div>
              ...
              <div className="text-[11px] font-bold text-slate-400 font-mono border-t border-slate-800/50 pt-2 flex items-center justify-between">
                ...
                <span className="text-cyan-500 text-[9px] bg-cyan-950/40 px-1.5 py-0.5 rounded border border-cyan-900/30">{t('verified_tag', 'Verified')}</span>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="space-y-6 max-w-4xl mx-auto">
        <div className="text-center space-y-1">
          <h2 className="text-xl font-black text-white tracking-wide">{t('faq_heading', 'Frequently Asked Questions')}</h2>
          <p className="text-xs text-slate-400">{t('faq_subheading', 'Clear breakdowns of professional chemical detailing and car care boundaries.')}</p>
        </div>
      ...
      <div className="bg-linear-to-br from-slate-900 to-slate-950 border border-slate-800 p-8 rounded-2xl text-center space-y-6 max-w-3xl mx-auto shadow-2xl relative overflow-hidden">
        <div className="space-y-2 relative z-10">
          <h3 className="text-xl font-black text-white tracking-wide">{t('cta_title', 'Ready to Transform Your Vehicle?')}</h3>
          <p className="text-xs text-slate-400 max-w-md mx-auto leading-relaxed">
            {t('cta_desc', 'Register your vehicle inside your account garage workspace profile and lock down an open detailing suite bay timing slot instantly.')}
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4 border-t border-slate-800/60 pt-6 max-w-sm mx-auto text-[11px] font-mono text-slate-500">
          <div>
            <p className="font-bold text-slate-400">{t('footer_hours_lbl', 'Studio Hours')}</p>
            <p className="mt-0.5">{t('footer_hours_val', 'Mon - Sat: 8AM - 6PM')}</p>
          </div>
          <div>
            <p className="font-bold text-slate-400">{t('footer_loc_lbl', 'Studio Location')}</p>
            <p className="mt-0.5">{t('footer_loc_val', 'Premium Curation Suite Bay A')}</p>
          </div>
        </div>
      </div>
